...
class YearlyIncomePieChart extends StatefulWidget {
  const YearlyIncomePieChart({Key? key}) : super(key: key);

  @override
  State<YearlyIncomePieChart> createState() => _YearlyIncomePieChartState();
}

class _YearlyIncomePieChartState extends State<YearlyIncomePieChart> {
  ...
  Map<String, double> getCategoryData(){
    ...
    for(var item in expenseIncomeList){
      ...
      if(catMap.containsKey(item.category)==false){
        catMap[item.category.toString()] =  item.income ?? 0;
      }
      else{
        double sum = 0;
